...
async function sayHello(): Promise<void> {
  ...
  try {
    ...
    const greetedPubkey = await PublicKey.createWithSeed(pubKey, GREETING_SEED, programId);
    ...
    if (greetedAccount === null) {
      ...
      console.log('Account creation transaction confirmed:', signature);
    } else {
      ...
      // Refresh blockhash before sending the "hello" transaction
      const { blockhash } = await connection.getLatestBlockhash();
      transaction.recentBlockhash = blockhash;
      transaction.feePayer = pubKey;
      ...
      console.log('Hello transaction confirmed:', signature);
    }
  } catch (error) {
